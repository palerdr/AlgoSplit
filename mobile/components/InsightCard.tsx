...
// Pre-generate random-ish dot opacities for a subtle shimmer texture
const DOT_ROWS = 14;
const DOT_COLS = 24;
...
for (let r = 0; r < DOT_ROWS; r++) {
  ...
  for (let c = 0; c < DOT_COLS; c++) {
    // Pseudo-random via simple hash, very low opacity
...
export default function InsightCard({ title, description, index = 0 }: InsightCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Very subtle shimmer line along top */}
        <View style={styles.topSheen} />

        {/* Ultra-subtle dot texture — barely visible glimmer */}
          ...
          {dotOpacities.map((row, ri) => (
            <View key={ri} style={styles.textureRow}>
              {row.map((op, ci) => (
                <View
                  key={ci}
                  style={[styles.textureDot, { opacity: op }]}
                />
              ))}
            </View>
          ))}
