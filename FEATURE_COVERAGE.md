...
**Status legend:** `client-only` = client function exists, no screen wired yet. The screens
team flips a row to `wired` when a screen consumes it. Every row currently starts at
`client-only` — nothing is omitted from the client.
...
| Method | Path | Source | Client function | Status |
| --- | --- | --- | --- | --- |
| POST | `/auth/signup` | api/routes/auth.py:33 | `auth.signup(email, password)` | client-only |
| POST | `/auth/login` | api/routes/auth.py:124 | `auth.login(email, password)` | client-only |
| GET | `/auth/user` | api/routes/auth.py:209 | `auth.me()` | client-only |
...
| POST | `/auth/logout` | api/routes/auth.py:384 | `auth.logout()` | client-only |
...
| GET | `/api/splits?include_exercises=` | api/routes/splits.py:401 | `splits.list(includeExercises?)` | client-only |
...
| PUT | `/api/splits/{split_id}/full` | api/routes/splits.py:584 | `splits.replace(splitId, split)` | client-only |
...
| Method | Path | Source | Client function | Status |
| --- | --- | --- | --- | --- |
| POST | `/api/workouts` | api/routes/workouts.py:152 | `workouts.create(payload)` | client-only |
| GET | `/api/workouts?limit=&offset=&days=` | api/routes/workouts.py:278 | `workouts.list(params?)` | client-only |
...
| Method | Path | Source | Client function | Status |
| --- | --- | --- | --- | --- |
| POST | `/api/analyze-split` | api/analysis_routes.py:94 | `analysis.analyzeSplit(request)` | client-only |
